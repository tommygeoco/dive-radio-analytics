...
| runtime `daily-attempts.json` | per Phoenix day: invocations, at most two whole-chain attempts, source states, commit, build timestamp, deployment, artifact hashes, outcomes, and the missing-source alert marker | run-daily | locked atomic state outside Git; 31 days kept | a missing initialized ledger fails closed; the alert marker prevents a repeated same-day recovery check from resending the same source warning |
