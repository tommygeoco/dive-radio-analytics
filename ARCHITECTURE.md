...
| snapshot | `scripts/restream/postlive-track.mjs snapshot` | `data/restream/postlive/<slug>.json` (append) + `data/restream/source-receipts.json` | no | the newest episode must return both YouTube uploads, both X accounts, and both X broadcasts before any history is appended; missing numbers stay absent, never zero |
