...
| Show health score, headline, seven checks, pros/cons, direction, outlook, as-of | `health` (incl. `ageDays`, `withheld`, per-measure `note` / `qualified` / `carried`, `direction`, `outlook`, `asOf`) | **health.mjs** (`health-v4`, deterministic checks; PRD v10) + model synthesis (prompt v6; deterministic fallback after two failures or with no key, stamped `provider: "deterministic"`) | snapshots, live events, analytics history/file, classified comments, `showTrend`, `baselines.launch` | §4; one entry per Phoenix day when ≥3 checks are available; promo-flagged lifts shown not scored; carried reads at half weight; direction = Theil–Sen over the last 5 clean episodes; outlook = last 3 clean first weeks; score within ±8 of the weighted mean; each bullet cites one fact; withheld after 7 days |
...
| | Show health (`health.mjs`, formula `health-v4`, prompt v6) | Episode health (`ratings.mjs`, algorithm `health21-v2`) + launch word (`baselines.launchReadFor`) |
|---|---|---|
