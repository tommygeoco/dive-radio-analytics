...
| Show health score, headline, six checks, pros/cons | `health` (incl. `ageDays`, `withheld`, per-measure `note`) | **health.mjs** (`health-v3`, deterministic checks) + model synthesis | snapshots, analytics history/file, classified comments, `showTrend`, `episode.health` | §4; one entry per Phoenix day when ≥3 checks are available; score within ±8 of the weighted mean; each bullet cites one fact; withheld after 7 days |
| "What matters" | `insights[]`, `insightsStale[]` | recommendations.mjs (store) else build-data rule insights | fact sheet over every store; rate facts carry `ageDays`/`basis` | every number token must exist in today's fact sheet or the item is held back; no item compares a young episode's rate with a finished one's |
...
| | Show health (`health.mjs`, formula `health-v3`, prompt v3) | Episode health (`ratings.mjs`, algorithm `health21-v2`) |
|---|---|---|
...
| Rendered | gauge + six-word diagnosis + today's read + evidence drill-in with each measure's note and each check's reason (`index.html` `buildHealth`) | carousel chip (silent when unscored), panel tile with notes / "Not compared" / the reason, neutral table column, Slack sequence of scored episodes only |
