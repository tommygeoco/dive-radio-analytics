...
| | Show health (`health.mjs`, formula `health-v3`, prompt v4) | Episode health (`ratings.mjs`, algorithm `health21-v2`) |
|---|---|---|
...
| Rendered | one strip: small gauge with its band in words, the six checks as words grouped by state (each name a tooltip drill with each measure's note and each check's reason), the headline whole, and Expand → the evidence, the saved scores on a fixed scale, and the do-next actions (`index.html` `buildHealth`) | carousel chip (silent when unscored), panel tile with notes / "Not compared" / the reason, neutral table column, Slack sequence of scored episodes only |
