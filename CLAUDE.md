...
- **Daily work is bounded and recoverable:** `run-daily.mjs` holds an exclusive run lock and records no more than two whole-chain attempts on one Phoenix day. The publisher checks every Git and Vercel result, retries at most once, and treats an unconfirmed live site as failure. `recover-publish.mjs` uses the reserved second attempt before 10:00 for a real failure, or at 12:15 when the morning build is current but the newest YouTube watch report is still pending, then proves freshness and exact bytes again.
