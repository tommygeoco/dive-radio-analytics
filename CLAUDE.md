...
```
postlive-discover → audience-archive → transcripts-pull → newsletter-promotion → postlive-track snapshot --all → yt-analytics-pull
...
- Daily snapshots refresh YouTube totals, X reach and resolved X broadcast plays for every aired active episode, without an age cutoff. The bare manual `snapshot` command still defaults to 60 days; the chain explicitly uses `--all`. Future/inactive episodes stay excluded. Freshness follows the full active registry, not just the newest episode. Missing upstream values remain absent or honestly stale; a current failed check never becomes a fake fresh count.
- A YouTube total can be revised downward by the source. Validator block 2 accepts a material decrease only when both raw observations prove the same registered upload, ready source envelopes, matching pull times and exact per-object counts. It reports the revision, keeps both historical rows, and serves the observed count. Malformed, missing or mismatched evidence and X decreases retain their existing gates; full source integrity and rebuild checks still block publication. Live-night Restream readings, frozen health scores and owner-imported LinkedIn data are unchanged.
...
- **Daily work is bounded and recoverable:** `run-daily.mjs` holds an exclusive run lock and records no more than two whole-chain attempts on one Phoenix day. The publisher checks every Git and Vercel result, retries at most once, and treats an unconfirmed live site as failure. A typed YouTube waiting result is handled after either platform attempt. `recover-publish.mjs` uses an available attempt at either 08:15 or 12:15 for any unproved production state; an honest YouTube wait at 08:15 reserves the run for 12:15. It then proves freshness and exact bytes again. If watch data still waits, the daily ledger marks and queues one alert for that Phoenix day; a later complete watch read plus production proof clears an undelivered stale line. `--proof-only` can verify production without capture or spending an attempt.
- **Alerts are acknowledged:** every producer uses `alert-queue.mjs`; corrupt or missing queue data fails loudly. `dive-alerts` runs every five minutes through `alerts.mjs --deliver`; sent lines remain pending until OpenClaw returns Slack's message receipt. The runtime preparation installs and verifies the pre-push hook on every checkout setup; manual release clones use `sh scripts/dev/install-hooks.sh`. Every publishing push also runs the full gate explicitly.
