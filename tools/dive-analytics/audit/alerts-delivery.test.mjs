...
try {
  reset([]);
  let calls = 0;
  assert.deepEqual(deliverPending({ queuePath: queue, target: "user:test", log: () => {}, send: () => { calls++; return { status: 0, stdout: "{}" }; } }), { sent: 0, receipts: [] });
  ...
  const result = deliverPending({
    ...
    send: (args) => {
      ...
      assert.throws(() => deliverPending({ queuePath: queue, target: "user:test", send: () => ({ status: 0, stdout: "{}" }) }), /already in use/);
...
console.log("alerts-delivery.test: empty, send failure, missing receipt, provider receipt, concurrent producer/deliverer, and promo alert checks pass");
