...
const config = { classifierVersion: 1, promptVersion: 1, promptHash: createHash('sha256').update(readFileSync(new URL('../../../scripts/restream/comments-classify-prompt.md', import.meta.url))).digest('hex'), model: 'fixture', provider: 'fixture' };
config.configHash = createHash('sha256').update(JSON.stringify({ version: 1, promptVersion: 1, promptHash: config.promptHash, model: config.model, provider: config.provider, vocabulary: THEME_VOCABULARY })).digest('hex');
