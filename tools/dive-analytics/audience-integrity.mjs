...
export function validateAudienceStore(store, { promptHash } = {}) {
  ...
  for (const [slug, episode] of Object.entries(store.episodes)) {
    ...
    for (const row of episode.list) {
      ...
      if (row.source === 'live-chat' && (!/^chat:[a-f0-9]{64}:\d+$/.test(row.id) || (row.url && !/^https:\/\/(?:x\.com|www\.youtube\.com|youtu\.be)\//.test(row.url)))) errors.push(`${slug}: live-chat provenance is invalid`);
