...
Owner: Tommy. Status: SCRIPT SIDE SHIPPED 2026-08-23 (same day — W17, W18,
W19 code + validator + local acceptance; see the implementation record).
The cron edits and the supervised end-to-end run are owner steps and remain
OPEN. Builds on v5 (recommendation engine, W15) and v6 (transcript moments,
W16). Goal: the morning publish can never be blocked by a stale model
artifact, and the model artifact can never go stale silently.
...
1. W17 script change + acceptance runs locally. — DONE 2026-08-23
...
3. W18 prompt/retry change, replay test. — code DONE 2026-08-23; the replay
   test needs ANTHROPIC_API_KEY, so it runs on the owner machine
4. W19 watchdog + midday cron. — script DONE 2026-08-23; the midday cron is
   owner-side
...
- **W17** in `recommendations.mjs`: `--prune` mode re-validates the saved
...
- **W18** in `recommendations.mjs`: the payload carries `allowedNumbers`
  ...
  error and the offending reply back to the model, up to three attempts
  total; the saved store records `attempts`. PROMPT_VERSION → 3.
...
- **W19** as `freshness.mjs` (sibling — `alerts.mjs` stays no-network by
