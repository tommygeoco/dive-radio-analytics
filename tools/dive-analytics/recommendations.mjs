...
//   - Failure exits nonzero after safely pruning stale saved items: when
...
//   recommendations (this script) → build-data → validate →
...
export function collectFacts(data = readJson(DATA_PATH)) {
  ...
  const shapeComparison = (e) => ({ episode: e.ep, ageDays: e.ageDays, comparisonBasis: basisOf(e) });
  ...
  for (const e of eps) {
    ...
    if (w?.curve?.length) {
      ...
      add(`open-start-E${e.ep}`, start.watching * 100, `E${e.ep} share still watching near the start`, shapeComparison(e));
      if (at2) add(`open-at2-E${e.ep}`, at2.watching * 100, `E${e.ep} share still watching at the 2% mark`, shapeComparison(e));
    }
  ...
  for (const e of eps) {
    ...
    if (s) {
      add(`open-floor-E${e.ep}`, s.openFloor, `E${e.ep} lowest share watching inside the first 5% of the video`, shapeComparison(e));
      add(`recovery-peak-E${e.ep}`, s.recoveryPeak, `E${e.ep} share watching at the early rebound peak`, shapeComparison(e));
      add(`mid-hold-E${e.ep}`, s.midHold, `E${e.ep} average share watching through the middle half`, shapeComparison(e));
      add(`end-hold-E${e.ep}`, s.endHold, `E${e.ep} share still watching near the end`, shapeComparison(e));
    }
    for (const mo of e.watch?.moments || []) {
      ...
      add(id, mo.points, mo.kind === "drop"
        ...
        : `E${e.ep} extra viewers of every 100 watching around ${pos}% of the way in`, shapeComparison(e));
...
const factEpisode = (fact) => fact.episode ?? (Number(/(?:^|-)E(\d+)(?:-|$)/.exec(fact.id)?.[1]) || null);
// All lifetime YouTube rate families need comparison metadata. The finished
// live-session hold-E# fact is deliberately distinct from hold-E#-position.
const EPISODE_RATE_ID = /^(?:(?:watched|subs1k|discovery|open-start|open-at2|open-floor|recovery-peak|mid-hold|end-hold)-E\d+(?:-|$)|(?:drop|hold)-E\d+-\d+$)/;

// Stamp the identity and comparison metadata needed to verify a bound item
// independently of later source changes. Old stores are never retro-bound.
export const stampedFacts = (facts) => facts.map(({id,value,kind,basis,episode,ageDays,comparisonBasis}) => ({id,value,
  ...(kind ? {kind} : {}), ...(basis ? {basis} : {}), ...(episode != null ? {episode} : {}),
  ...(ageDays != null ? {ageDays} : {}), ...(comparisonBasis ? {comparisonBasis} : {})}));

function boundFactsFor(item, facts, requireFactIds) {
  if (!Object.hasOwn(item, "factIds")) {
    if (requireFactIds) throw new Error(`${item.id}: new model output requires factIds`);
    return null;
  }
  if (!Array.isArray(item.factIds) || !item.factIds.length || item.factIds.some(id => typeof id !== "string" || !id)
      || new Set(item.factIds).size !== item.factIds.length) throw new Error(`${item.id}: factIds must be unique nonempty IDs`);
  return item.factIds.map(id => {
    const matches = facts.filter(f => f.id === id);
    if (matches.length !== 1) throw new Error(`${item.id}: unknown or ambiguous cited fact ${id}`);
    const fact = matches[0];
    if (!Number.isFinite(fact.value)) throw new Error(`${item.id}: cited fact ${id} has no finite value`);
    if (EPISODE_RATE_ID.test(id) || fact.kind === "episode-rate" || fact.comparisonBasis != null) {
      const basis = fact.comparisonBasis ?? fact.basis;
      if (!["young", "mature"].includes(basis) || !Number.isInteger(fact.episode) || fact.episode <= 0
          || !Number.isFinite(fact.ageDays) || fact.ageDays < 0
          || (fact.basis != null && fact.basis !== basis)) {
        throw new Error(`${item.id}: cited rate ${id} has missing or invalid comparison metadata`);
      }
    }
    return fact;
  });
...
function checkItem(item, allowed, seen, facts = [], { requireFactIds = false } = {}) {
  ...
  const bound = boundFactsFor(item, facts, requireFactIds);
  const citationAllowed = bound ? allowedTokens(bound) : allowed;
  const basesCited = new Set(bound ? bound.map(f => f.comparisonBasis || (f.kind === "episode-rate" ? f.basis : null)).filter(Boolean) : []);
  for (const key of ["text", "recommendation"]) {
    ...
    const numericText = bound ? value.replace(/\bE(\d+)\b/g, (label, ep) => {
      if (!bound.some(f => factEpisode(f) === Number(ep))) throw new Error(`${item.id}: episode ${label} is not in the cited facts`);
      return "";
    }) : value;
    for (const token of numberTokens(numericText)) {
      if (!citationAllowed.has(token)) throw new Error(`${item.id}: number ${token} is not in the ${bound ? "cited facts" : "fact sheet"}`);
    }
    ...
    if (!bound) for (const token of numberTokens(value.replace(/\bE\d+\b/g, ""))) {
  ...
  if (bound && item.caveat != null) {
    const text = item.caveat.replace(/\bE(\d+)\b/g, (label, ep) => {
      if (!bound.some(f => factEpisode(f) === Number(ep))) throw new Error(`${item.id}: episode ${label} is not in the cited facts`);
      return "";
    });
    for (const token of numberTokens(text)) if (!citationAllowed.has(token)) throw new Error(`${item.id}: caveat number ${token} is not in the cited facts`);
  }
}
...
export function validateItem(item, facts, allowed = allowedTokens(facts), options = {}) {
  checkItem(item, allowed, new Set(), facts, options);
  return item;
}

export function validateItems(items, facts, options = {}) {
  ...
  const seen = new Set();
  for (const item of items) checkItem(item, allowed, seen, facts, options);
  return items;
}

export function validateGeneratedItems(items, facts) {
  validateItems(items, facts, {requireFactIds:true});
  if (items.length !== TOP_N) throw new Error(`exactly ${TOP_N} ranked items required, got ${items.length}`);
  return items;
}
...
export function pruneStore(sheet) {
  ...
  for (const item of store.items) {
    try { checkItem(item, allowed, seen, sheet.facts, {requireFactIds:store.promptVersion >= 5}); kept.push(item); }
  ...
  if (!prunedIds.length) {
    ...
    const facts = stampedFacts(sheet.facts);
  ...
  saveAtomic(STORE_PATH, {
    ...
    facts: stampedFacts(sheet.facts),
